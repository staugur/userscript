...
// @version      0.1.0
// @description  站酷网(zcool.com.cn)图片批量下载到本地
// @author       staugur
// @match        http*://www.zcool.com.cn/work/*.html
...
// @date         2019-04-27
// @modified     2019-04-27
// @github       https://github.com/staugur/grab_huaban_board/blob/master/grab_zcool_work.user.js
...
(function() {
        ...
        layer.tab({
            ...
            success: function(layero, index) {
                ...
                body.context.getElementById("reset_notice_status").onclick = function() {
                    var storage = new StorageMix("grab_zcool_work");
                    storage.clear();
                    layer.msg("重置成功", {
                        icon: 1
                    });
    ...
    function setupReceiveTo(type, value) {
        var es = new StorageMix("grab_zcool_work_remind_email");
        var ms = new StorageMix("grab_zcool_work_remind_mobile");
        var ts = new StorageMix("grab_zcool_work_token");
        if (type === 'email') {
    ...
    function getReceiveBy(type) {
        var str = '',
            es = new StorageMix("grab_zcool_work_remind_email"),
            ms = new StorageMix("grab_zcool_work_remind_mobile"),
            ts = new StorageMix("grab_zcool_work_token");
        if (type === 'email') {
            str = es.get();
        } else if (type === 'mobile') {
            str = ms.get();
    ...
    */
    ...
    function downloadWork() {
        var work_id = location.pathname.split("/")[location.pathname.split("/").length-1].split("=")[0];
        if (work_id) {
            console.group("站酷网下载-当前作品：" + work_id);
            var author_url = document.getElementsByClassName("author-info-title")[0].getElementsByTagName("a")[0].href;
            var user_id = author_url.split("://")[author_url.split("://").length-1].split(".zcool.com.cn")[0];
            /*
            f=getElementsByClassName("work-show-box")

f[0].getElementsByTagName("img")[0]

# 下一页
f=document.getElementById("biuuu_page").getElementsByTagName("a")
*/
            //get first pin data
            jQuery.ajax({
    ...
    function showNotice() {
        jQuery.ajax({
            url: "https://open.saintic.com/CrawlHuaban/notice?catalog=4",
            type: "GET",
            success: function(res) {
                if (res.code === 0) {
    ...
    */
    ...
    if (work_id) {
        //当前在专辑地址下
        var board_text = "下载此作品",
            setup_text = "站酷网设置";
        //当前是PC版，不予支持Mobile版
        var dhd = document.getElementsByClassName("work-details-wrap")[0].getElementsByClassName("work-head-box")[0].getElementsByClassName("head-data-show")[0];
        //插入下载专辑按钮
        if (isContains(dhd.innerText, board_text) === false) {
            var tmpHtml = '<a href="javascript:;" id="setupRemind" class="ertical-line">' + setup_text + '</a>' +
                '<a href="javascript:;" id="downloadWork" class="ertical-line">' + board_text + '</a>';
            dhd.style.width = 'auto';
            dhd.insertAdjacentHTML('afterbegin', tmpHtml);
        }
        // 监听设置提醒按钮
        ...
        //监听专辑点击下载事件
        document.getElementById("downloadWork").onclick = function() {
            showNotice();
            downloadWork();
        };
    }
